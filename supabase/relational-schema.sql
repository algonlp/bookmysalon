-- Full normalized Supabase schema for BookMySalon.
--
-- Important:
-- 1. The current application still reads/writes the JSONB sync tables in supabase/schema.sql.
-- 2. This file creates the full relational table structure for a complete migration away from JSON payload storage.
-- 3. Run this only after backing up your current Supabase data.
